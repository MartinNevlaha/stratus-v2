...
func (e *Engine) RunAnalysis(ctx context.Context) error {
	...
	start := time.Now()
	slog.Info("insight routing: analysis started")
	...
	slog.Debug("insight routing: loaded metrics",
		"agents", len(agentMetrics),
		"workflows", len(workflowMetrics))

	if len(agentMetrics) == 0 && len(workflowMetrics) == 0 {
		slog.Info("insight routing: no metrics available for analysis")
		return nil
	}
	...
	for _, analyzer := range e.analyzers {
		...
		slog.Debug("insight routing: analyzer completed",
			"analyzer", analyzer.Name(),
			"recommendations", len(recommendations))
	}

	slog.Debug("insight routing: total recommendations generated", "count", len(allRecommendations))
	...
	slog.Info("insight routing: analysis complete",
