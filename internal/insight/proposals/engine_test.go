...
func TestGenerateProposalFromAgentPerformanceDropLegacyEvidence(t *testing.T) {
	pattern := patterns.Pattern{
		ID:          "test-pattern-legacy-agent",
		Type:        patterns.PatternAgentPerformanceDrop,
		Timestamp:   time.Now().UTC(),
		Severity:    patterns.SeverityMedium,
		Description: "Legacy evidence keys",
		Evidence: map[string]any{
			"drop_rate":              0.22,
			"worst_performing_agent": "legacy-agent",
		},
		Frequency:  2,
		Confidence: 0.72,
	...
	proposal, err := (&AgentPerformanceDropGenerator{}).Generate(pattern)
	if err != nil {
		t.Fatalf("expected generator to accept legacy evidence, got error: %v", err)
	}
	if proposal == nil {
		t.Fatal("expected proposal, got nil")
	}
	if agentID, ok := proposal.Recommendation["agent_id"].(string); !ok || agentID != "legacy-agent" {
		t.Fatalf("expected legacy agent ID to be used, got %v", proposal.Recommendation["agent_id"])
	}
}

func TestGenerateProposalFromWorkflowDurationSpikeLegacyEvidence(t *testing.T) {
	pattern := patterns.Pattern{
		ID:          "test-pattern-legacy-duration",
		Type:        patterns.PatternWorkflowDurationSpike,
		Timestamp:   time.Now().UTC(),
		Severity:    patterns.SeverityHigh,
		Description: "Legacy duration evidence keys",
		Evidence: map[string]any{
			"multiplier":            2.4,
			"slowest_workflow_type": "spec",
		},
		Frequency:  2,
		Confidence: 0.7,
		FirstSeen:  time.Now().Add(-6 * time.Hour).UTC(),
	...
	proposal, err := (&WorkflowDurationSpikeGenerator{}).Generate(pattern)
	if err != nil {
		t.Fatalf("expected generator to accept legacy duration evidence, got error: %v", err)
	}
	if proposal == nil {
		t.Fatal("expected proposal, got nil")
	}
	if proposal.Type != ProposalTypeRetryPolicyAdjust {
		t.Fatalf("expected retry policy adjustment for 2.4x multiplier, got %s", proposal.Type)
	}
	if workflowType, ok := proposal.Recommendation["workflow_type"].(string); !ok || workflowType != "spec" {
		t.Fatalf("expected workflow_type from legacy evidence, got %v", proposal.Recommendation["workflow_type"])
	}
}
