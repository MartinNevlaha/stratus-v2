...
func (m *mockEventQuery) GetEventsByTypesInTimeRange(ctx context.Context, eventTypes []string, start, end time.Time, limit int) ([]EventForDetection, error) {
	m.requestedTypesLog = append([]string(nil), eventTypes...)
	return m.events, nil
}
...
func TestAgentPerformanceDropDetector(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectPattern && pattern != nil {
				if _, ok := pattern.Evidence["performance_drop"]; !ok {
					t.Error("expected canonical performance_drop evidence key")
				}
				if _, ok := pattern.Evidence["agent_id"]; !ok {
					t.Error("expected canonical agent_id evidence key")
				}
			}
			if !tt.expectPattern && pattern != nil {
				t.Errorf("expected no pattern, got %+v", pattern)
...
func TestWorkflowDurationSpikeDetector(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectPattern && pattern != nil {
				if _, ok := pattern.Evidence["duration_multiplier"]; !ok {
					t.Error("expected canonical duration_multiplier evidence key")
				}
				if _, ok := pattern.Evidence["affected_workflow"]; !ok {
					t.Error("expected canonical affected_workflow evidence key")
				}
			}
			if !tt.expectPattern && pattern != nil {
...
func TestEngineRequestsWorkflowLifecycleEvents(t *testing.T) {
	mockQuery := &mockEventQuery{
		events: []EventForDetection{
			{Type: "workflow.started", Timestamp: time.Now()},
			{Type: "workflow.phase_transition", Timestamp: time.Now()},
			{Type: "workflow.completed", Timestamp: time.Now()},
		},
	}
	mockStore := &mockPatternStore{}
	config := DefaultDetectionConfig()
	config.MinEventsForDetection = 1

	engine := NewEngine(mockQuery, mockStore, config)
	if err := engine.RunDetection(context.Background()); err != nil {
		t.Fatalf("RunDetection failed: %v", err)
	}

	hasStarted := false
	hasTransition := false
	for _, eventType := range mockQuery.requestedTypesLog {
		if eventType == "workflow.started" {
			hasStarted = true
		}
		if eventType == "workflow.phase_transition" {
			hasTransition = true
		}
	}
	if !hasStarted {
		t.Fatal("expected workflow.started in requested event types")
	}
	if !hasTransition {
		t.Fatal("expected workflow.phase_transition in requested event types")
	}
}
