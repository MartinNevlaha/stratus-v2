...
func (e *Engine) RunDetection(ctx context.Context) error {
	...
	start := time.Now()
	slog.Info("insight patterns: detection started")

	end := time.Now()
	startTime := end.Add(-time.Duration(e.config.EventWindowHours) * time.Hour)

	allEventTypes := []string{
		"workflow.started", "workflow.phase_transition",
	...
	slog.Debug("insight patterns: loaded events", "count", len(events), "window_h", e.config.EventWindowHours)

	if len(events) < e.config.MinEventsForDetection {
		slog.Info("insight patterns: insufficient events for detection", "count", len(events), "min", e.config.MinEventsForDetection)
		return nil
	}
	...
	for _, detector := range e.detectors {
		func() {
			defer func() {
				if r := recover(); r != nil {
					slog.Error("insight patterns: detector panic recovered", "detector", detector.Name(), "panic", r)
				}
			}()
			...
			if pattern != nil {
				...
				slog.Info("insight patterns: pattern detected",
					"type", pattern.Type,
	...
	for _, pattern := range detectedPatterns {
		...
		if err != nil {
			slog.Error("insight patterns: failed to save pattern", "type", pattern.Type, "error", err)
			continue
		}
	...
	slog.Info("insight patterns: detection complete",
