...
func (e *Engine) RunComputation(ctx context.Context) error {
	...
	start := time.Now()
	slog.Info("openclaw scorecards: computation started")
	...
	for _, window := range windows {
		...
		if err != nil {
			slog.Error("openclaw scorecards: window computation failed", "window", window, "error", err)
			continue
		}
	...
	slog.Info("openclaw scorecards: computation complete",
...
func (e *Engine) computeForWindow(ctx context.Context, window Window) ([]AgentScorecard, []WorkflowScorecard, error) {
	...
	slog.Debug("openclaw scorecards: loaded events", "window", window, "count", len(events))
...
func (e *Engine) computeAgentScorecards(
	...
	if len(agentNames) == 0 {
		slog.Debug("openclaw scorecards: no agents found in events")
		return nil
	}
	...
	for _, agentName := range agentNames {
		...
		if card.TotalRuns > 0 {
			...
			if err := e.scorecardStore.SaveAgentScorecard(ctx, card); err != nil {
				slog.Error("openclaw scorecards: failed to save agent scorecard",
	...
	slog.Debug("openclaw scorecards: computed agent scorecards", "count", len(cards))
	return cards
}
...
func (e *Engine) computeWorkflowScorecards(
	...
	if len(workflowTypes) == 0 {
		slog.Debug("openclaw scorecards: no workflow types found in events")
		return nil
	}
	...
	for _, wfType := range workflowTypes {
		...
		if card.TotalRuns > 0 {
			...
			if err := e.scorecardStore.SaveWorkflowScorecard(ctx, card); err != nil {
				slog.Error("openclaw scorecards: failed to save workflow scorecard",
	...
	slog.Debug("openclaw scorecards: computed workflow scorecards", "count", len(cards))
	return cards
}
