...
func NewEngine(patternStore patterns.PatternStore, proposalStore ProposalStore, config EngineConfig) *Engine {
	if patternStore == nil {
		slog.Warn("openclaw proposals: pattern store is nil, engine will not function properly")
	}
	if proposalStore == nil {
		slog.Warn("openclaw proposals: proposal store is nil, engine will not function properly")
	}
...
func (e *Engine) RunGeneration(ctx context.Context) error {
	...
	start := time.Now()
	slog.Info("openclaw proposals: generation started")
	...
	slog.Debug("openclaw proposals: loaded patterns", "count", len(patternsList))

	if len(patternsList) == 0 {
		slog.Info("openclaw proposals: no patterns available for generation")
		return nil
	}
	...
	slog.Debug("openclaw proposals: generated proposals", "count", len(proposals))
	...
	slog.Info("openclaw proposals: generation complete",
