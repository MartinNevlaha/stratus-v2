...
func (e *Engine) RunAnalysis() error {
	...
	if len(dailyMetrics) == 0 {
		log.Println("insight: no metrics data available")
		return nil
	}
	...
	if state != nil {
		...
		if err := e.database.UpdateInsightState(state); err != nil {
			log.Printf("insight: failed to update state: %v", err)
		}
	}
...
func (e *Engine) savePatternIfNew(pattern *db.InsightPattern) error {
	...
	if existing != nil {
		...
		if err := e.database.UpdateInsightPattern(existing); err != nil {
			return fmt.Errorf("update pattern: %w", err)
		}
		log.Printf("insight: updated existing pattern: name=%s frequency=%d", pattern.PatternName, existing.Frequency)
		return nil
	}
	...
	log.Printf("insight: detected new pattern: type=%s name=%s confidence=%.2f",
