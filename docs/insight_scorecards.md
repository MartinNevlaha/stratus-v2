...
Scorecards provide long-term operational performance summaries for agents and workflows in Stratus. They enable Insight to reason about system quality in a durable way, answering questions like:
...
```
GET /api/insight/scorecards/agents
```
...
```
GET /api/insight/scorecards/agents/:name
```
...
```
GET /api/insight/scorecards/workflows
```
...
```
GET /api/insight/scorecards/workflows/:type
```
...
```
POST /api/insight/scorecards/compute
```
...
```
GET /api/insight/scorecards/highlights
```
...
The Insight dashboard displays scorecards with:
...
```
insight/events → scorecards/engine.go → scorecards/store.go → database
                      ↓
