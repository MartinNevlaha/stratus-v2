...
**Important**: Routing recommendations are **advisory only**. Insight does NOT automatically change routing behavior. All routing decisions require human review and approval.
...
```
GET /api/insight/routing/recommendations
```
...
```
GET /api/insight/routing/recommendations/{id}
```

### Trigger Analysis

```
POST /api/insight/routing/analyze
```
...
The Insight dashboard includes a **Routing Recommendations** section showing:
...
| File | Purpose |
|------|---------|
| `internal/insight/routing/models.go` | Data models and confidence calculation |
| `internal/insight/routing/analyzer.go` | Analyzer implementations |
| `internal/insight/routing/engine.go` | Engine orchestration |
| `internal/insight/routing/store.go` | Database persistence |
| `db/schema.go` | Database schema |
| `db/insight.go` | Database operations |
| `api/routes_insight.go` | HTTP handlers |
| `insight/engine.go` | Main engine integration |
