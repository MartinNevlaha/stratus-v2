# Insight Event Ingestion Layer

The Insight Event Ingestion Layer provides real-time observability into the Stratus system by capturing and persisting events from various system components.

## Overview

Insight observes system behavior through events emitted by:
...
CREATE INDEX idx_insight_events_type ON insight_events(type);
CREATE INDEX idx_insight_events_timestamp ON insight_events(timestamp DESC);
```
...
The Insight engine subscribes to the event bus to observe system behavior:

```go
engine := insight.NewEngineWithEvents(database, cfg, eventBus)
```
...
- Database writes are performed in the background by Insight
