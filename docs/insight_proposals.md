# Insight Proposal Engine

## Overview

The Insight Proposal Engine transforms detected system patterns into **structured improvement proposals**. These proposals represent suggested improvements to the system, such as:
...
Proposals are generated automatically during scheduled Insight analysis:
...
Configure interval in Insight config (default: 1 hour).
...
```bash
POST /api/insight/proposals/generate
```
...
```bash
GET /api/insight/proposals?limit=50&status=detected&min_confidence=0.6
```
...
CREATE INDEX idx_insight_proposals_type ON insight_proposals(type);
CREATE INDEX idx_insight_proposals_status ON insight_proposals(status);
CREATE INDEX idx_insight_proposals_pattern ON insight_proposals(source_pattern_id);
CREATE INDEX idx_insight_proposals_created ON insight_proposals(created_at DESC);
```
...
1. **Insight Scheduler** - Triggers proposal generation after pattern detection
2. **Insight Engine** - Manages proposal engine lifecycle
...
```
INFO insight proposals: generation started
```
...
```
INFO insight proposals: generation complete generated=5 saved=3 deduplicated=2 duration_ms=234
```
...
**Solution:** Check Insight logs for details.
...
The Insight Proposal Engine provides:
