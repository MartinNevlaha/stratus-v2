...
func (d *DB) FindPatternByName(patternName string) (*OpenClawPattern, error) {
	row := d.sql.QueryRow(`
...
func (d *DB) UpdateOpenClawPattern(pattern *OpenClawPattern) error {
	examplesBytes, err := json.Marshal(pattern.Examples)
	if err != nil {
		return fmt.Errorf("marshal examples: %w", err)
	}
...
func (d *DB) ListOpenClawPatterns(patternType string, severity string, minConfidence float64, limit int) ([]OpenClawPattern, error) {
	if limit <= 0 {
		limit = 100
	}
...
func (d *DB) SaveOpenClawProposal(proposal *OpenClawProposal) error {
...
func (d *DB) GetOpenClawProposalByID(id string) (*OpenClawProposal, error) {
	row := d.sql.QueryRow(`
...
func (d *DB) ListOpenClawProposals(proposalType string, status string, riskLevel string, minConfidence float64, limit int, offset int) ([]OpenClawProposal, error) {
	if limit <= 0 {
		limit = 100
	}
	if offset < 0 {
	...
	var proposals []OpenClawProposal
	for rows.Next() {
		var p OpenClawProposal
		var evidenceJSON, recommendationJSON string
		var decisionReason sql.NullString
...
func (d *DB) FindSimilarOpenClawProposal(proposalType string, sourcePatternID string, affectedEntity string, withinHours int) (*OpenClawProposal, error) {
	row := d.sql.QueryRow(`
		SELECT id, type, status, title, description, confidence, risk_level,
		       source_pattern_id, evidence, recommendation, decision_reason, created_at, updated_at
		FROM openclaw_proposals
...
func (d *DB) UpdateOpenClawProposalStatus(id string, status string) error {
	now := time.Now().UTC().Format(time.RFC3339Nano)
	_, err := d.sql.Exec(`
...
func (d *DB) UpdateOpenClawProposalStatusWithReason(id string, status string, reason string) error {
	now := time.Now().UTC().Format(time.RFC3339Nano)
	_, err := d.sql.Exec(`
		UPDATE openclaw_proposals
...
type OpenClawDashboardSummary struct {
...
func (d *DB) GetOpenClawDashboardSummary() (*OpenClawDashboardSummary, error) {
	summary := &OpenClawDashboardSummary{
...
func (d *DB) SaveAgentScorecard(card *AgentScorecard) error {
	...
	_, err := d.sql.Exec(`
		INSERT INTO openclaw_agent_scorecards 
...
func (d *DB) SaveWorkflowScorecard(card *WorkflowScorecard) error {
	...
	_, err := d.sql.Exec(`
		INSERT INTO openclaw_workflow_scorecards 
...
func (d *DB) ListAgentScorecards(window string, sortBy string, sortDir string, limit int) ([]AgentScorecard, error) {
	...
	query := fmt.Sprintf(`
		SELECT id, agent_name, window, window_start, window_end, total_runs, success_rate, failure_rate,
		       review_pass_rate, rework_rate, avg_cycle_time_ms, regression_rate, confidence_score, trend, created_at, updated_at
		FROM openclaw_agent_scorecards
		WHERE window = ?
		ORDER BY %s %s
		LIMIT ?
	`, sortBy, sortDir)
...
func (d *DB) ListWorkflowScorecards(window string, sortBy string, sortDir string, limit int) ([]WorkflowScorecard, error) {
	...
	query := fmt.Sprintf(`
		SELECT id, workflow_type, window, window_start, window_end, total_runs, completion_rate, failure_rate,
		       review_rejection_rate, rework_rate, avg_duration_ms, confidence_score, trend, created_at, updated_at
		FROM openclaw_workflow_scorecards
		WHERE window = ?
		ORDER BY %s %s
		LIMIT ?
	`, sortBy, sortDir)
...
func (d *DB) SaveRoutingRecommendation(rec *RoutingRecommendation) error {
	...
	_, err = d.sql.Exec(`
		INSERT INTO openclaw_routing_recommendations 
...
func (d *DB) ListRoutingRecommendations(filters RoutingRecommendationFilters, limit int) ([]RoutingRecommendation, error) {
	...
	query := `
		SELECT id, workflow_type, recommendation_type, recommended_agent, current_agent,
		       confidence, risk_level, reason, evidence, observations, created_at
		FROM openclaw_routing_recommendations
		WHERE 1=1
	`
	args := []any{}
