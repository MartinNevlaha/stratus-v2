...
func (m *MetricsCollector) RecordTaskComplete(workflowID string, taskIndex int, success bool) error {
	value := 1.0
	if !success {
		value = 0.0
	}
...
func (m *MetricsCollector) RecordPhaseTransition(workflowID string, fromPhase Phase, toPhase Phase, duration time.Duration) error {
	err := m.db.RecordMetric(workflowID, "workflow", "phase_duration", float64(duration.Milliseconds()), map[string]any{
...
func (m *MetricsCollector) RecordTaskComplete(workflowID string, taskIndex int, agentID string, success bool) error {
	value := 1.0
	if !success {
		value = 0.0
	}
