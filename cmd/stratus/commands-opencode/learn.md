---
description: "Analyze the codebase for repeating patterns and generate learning proposals (rules, ADRs, skills). Requires stratus serve to be running."
---

# Learning Workflow

Analyze the codebase for repeating patterns and anti-patterns, create learning candidates, and generate proposals for improvements (rules, ADRs, skill templates).
...
For each high-confidence candidate, generate a proposal with **full file content** and target path. The server will automatically write the file when the user accepts it in the dashboard.
...
**Important:** `proposed_content` and `proposed_path` are required for auto-apply to work. Without them, accepting the proposal in the dashboard only marks it as decided — no file is written.
...
Tell the user proposals are ready in the **Learning tab** of the stratus dashboard. They can accept/reject/snooze each one; accepting automatically writes the file and re-indexes governance.
