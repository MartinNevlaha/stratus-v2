# OpenClaw Stabilization Plan

## Executive Summary

The OpenClaw integration has **critical runtime bugs** that will cause immediate panics on startup. The module cannot function in its current state due to:
...
```go
fmt.Println("OpenClaw engine started")           // engine.go:52
fmt.Println("OpenClaw: No metrics data available") // analysis.go:25
...
#### 9. No Tests
**Location:** `openclaw/*_test.go` - doesn't exist
...
func (e *Engine) Start(ctx context.Context) error {
    ...
    go func() {
        ...
        if err := e.scheduler.Run(e.ctx); err != nil && err != context.Canceled {
            log.Printf("openclaw: scheduler stopped with error: %v", err)
        }
    }()
...
func (e *Engine) Stop() {
    ...
    log.Println("openclaw: engine stopped")
}
```
...
// Pattern detection
log.Printf("openclaw: detected pattern: type=%s name=%s confidence=%.2f",
    pattern.PatternType, pattern.PatternName, pattern.Confidence)

// Proposal generation
log.Printf("openclaw: generated proposal: type=%s title=%s confidence=%.2f",
    proposal.Type, proposal.Title, proposal.Confidence)

// Errors
log.Printf("openclaw: get daily metrics failed: %v", err)
log.Printf("openclaw: save pattern failed: %v", err)
```
...
if err != nil {
    log.Printf("openclaw: failed to count recent proposals: %v", err)
} else if recentProposals >= e.config.MaxProposals {
    log.Printf("openclaw: skipping proposals, max daily limit reached: %d", recentProposals)
    proposals = nil
}
```
...
func TestEngineStartStop(t *testing.T) {
    ...
    engine := openclaw.NewEngine(db, cfg)
...
**File:** `openclaw/analysis_test.go`
...
func TestAnalyzeMetrics(t *testing.T) {
    ...
    patterns, err := db.ListOpenClawPatterns("", 0, 100)
    if err != nil {
...
**Mitigation:** Add `/api/openclaw/health` endpoint that returns:
- Running status
...
5. **Context Propagation:** The main.go doesn't currently pass a context to `openclaw.Start()`. Should I:
